...
export default function ContactLeadsManager() {
  ...
  const loadContactLeads = async () => {
    try {
      ...
      const response = await fetch(`/api/workspace/crm/contact-leads?${params}`);
  ...
  const handleStatusUpdate = async (leadId: string, newStatus: string) => {
    try {
      ...
      const response = await fetch('/api/workspace/crm/contact-leads', {
        method: 'PUT',
  ...
  const handleDeleteLead = async (leadId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/workspace/crm/contact-leads?id=${leadId}`, {
        method: 'DELETE'
      });
