-- Migración para agregar soporte de múltiples expedientes por proyecto
-- Fecha: 2024-01-XX
...
WHERE table_name = 'project_expedientes' 
...
SELECT 
    ...
    p.dgro_file_number as old_expediente,
    pe.expediente_number as new_expediente,
    pe.expediente_type,
    pe.status
FROM projects p
LEFT JOIN project_expedientes pe ON p.id = pe.project_id
WHERE p.dgro_file_number IS NOT NULL
ORDER BY p.name;
*/

-- Ejemplo de inserción de nuevo expediente
/*
INSERT INTO project_expedientes (
    project_id,
    expediente_number,
    expediente_type,
    status,
    ...
    (SELECT id FROM projects LIMIT 1), -- Usar un project_id real existente
    'EX-2024-12345678-GCABA-DGIUR',
    'DGIUR',
    'En trámite',
    '2024-01-15',
    'Expediente para consulta DGIUR'
);
*/